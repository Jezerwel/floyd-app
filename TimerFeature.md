...
The app sends a `feedMs` parameter with the `start_feed` MQTT command. The ESP32 firmware runs the auger for the specified duration as part of its motor state machine.
...
Controls tab publishes MQTT command:
  {"action":"start_feed","feedMs":5000,"augerSpeed":768,...}
        │
        ▼
ESP32 receives command → enters PRE_SPIN → FEEDING (5s) → POST_SPIN → IDLE
```
