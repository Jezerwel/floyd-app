...
const MOTOR_STATE_CONFIG: Record<
	string,
	{
		label: string;
		icon: string;
		colorKey: "success" | "primary" | "warning" | "error" | "muted";
	}
> = {
...
export default function DashboardScreen() {
	...
	const skeletonOpacity = useRef(
		new Animated.Value(isConnected ? 0 : 1),
	).current;
	const dataOpacity = useRef(new Animated.Value(isConnected ? 1 : 0)).current;
	const disconnectScale = useRef(new Animated.Value(1)).current;

	// Fade skeletons out and data in when connected
	useEffect(() => {
		if (isConnected) {
			Animated.parallel([
	...
	}, [isConnected, skeletonOpacity, dataOpacity]);

	// Scale down + overlay when connection lost
	const showDisconnectOverlay = wasConnected && !isConnected;
	useEffect(() => {
		if (showDisconnectOverlay) {
			Animated.spring(disconnectScale, {
				toValue: 0.97,
				useNativeDriver: true,
			}).start();
		} else {
			disconnectScale.setValue(1);
		}
	}, [showDisconnectOverlay, disconnectScale]);
	...
	return (
		...
		>
			...
			>
				...
				{/* Skeleton cards during connecting */}
				...
				<Animated.View
					style={{
						opacity: dataOpacity,
						transform: [{ scale: disconnectScale }],
					}}
				>
					...
					{showDisconnectOverlay && (
						<View
							style={[
								styles.disconnectOverlay,
								{ backgroundColor: colors.warning + "20" },
							]}
						>
							<View style={styles.disconnectOverlayContent}>
								<IconSymbol
									name="wifi.slash"
									size={24}
									color={colors.warning}
								...
								{esp32Status === "disconnected" && (
									...
									>
										<Text style={[styles.helpText, { color: colors.text }]}>
											The feeder was found on the network but is not responding.
											Check power.
										</Text>
									</View>
								)}
							...
							<View style={styles.capacityInfo}>
								...
								<View style={styles.sensorInfo}>
									...
									{!isUltrasonicSensorConnected && isConnected && (
										<Text
											style={[styles.sensorWarning, { color: colors.error }]}
										>
...
const styles = StyleSheet.create({
	...
	disconnectOverlayContent: {
		flexDirection: "row",
		alignItems: "center",
		gap: 10,
	},
	disconnectOverlayText: {
		fontSize: 15,
		fontWeight: "600",
	},
});
